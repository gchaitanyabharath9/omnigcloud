...
groups:
\begin{itemize}
\item name: adaptiveTEMP_USpolicy
\end{itemize}
    rules:
\begin{itemize}
\item alert: LatencySpike
\end{itemize}
        expr: histogramTEMP_USquantile(0.99, httpTEMP_USrequestTEMP_USdurationTEMP_USseconds) > 0.5
        for: 1m
        annotations:
          action: enableTEMP_USaggressiveTEMP_UScaching
\end{verbatim}

\textbf{Policy Engine (OPA):}
\begin{verbatim}
package adaptiveTEMP_USpolicy

enableTEMP_USaggressiveTEMP_UScaching {
  input.alert.name == "LatencySpike"
  input.metrics.p99TEMP_USlatency > 500
}

action := "cacheTEMP_USttlTEMP_USincrease" {
  enableTEMP_USaggressiveTEMP_UScaching
}
\end{verbatim}
