...
allowed\_registries := ["gcr.io/company", "docker.io/company"]

deny[msg] {
  input.request.kind.kind == "Pod"
  container := input.request.object.spec.containers[\_]
  image := container.image
  not startswith(image, allowed\textit{registries[}])
...
test\textit{deny}missing\textit{memory}limit {
  input := {
    "request": {
      "kind": {"kind": "Pod"},
      "object": {
...
\end{itemize}
        run: curl -L -o opa https://openpolicyagent.org/downloads/latest/opa\textit{linux}amd64
\begin{itemize}
\item name: Run Tests
\end{itemize}
...
\end{itemize}
    ...
    bundles:
      authz:
        ...
        polling:
          min\textit{delay}seconds: 60
          max\textit{delay}seconds: 120
\texttt{`}
...
argocd\textit{app}sync\_total{phase="OutOfSync"} = 45

\# Time to reconciliation
argocd\textit{app}reconcile\textit{duration}seconds{quantile="0.99"} = 8.2
\texttt{`}
...
\end{itemize}
  ...
  condition: >
    outbound and
    not fd.sip in (allowed\_ips) and
    container.id != host
  output: >
...
\end{itemize}
  expr: |
    increase(kubernetes\textit{audit}event\_total{
...
$$ \text{Compliance}(S\textit{{infra}) = \bigwedge}{r \in S\textit{{infra}} \bigwedge}{p \in P} p(r) $$
...
identity = {"provider": "aws", "user": user, "role": role} {
    ...
    regex.match("arn:aws:iam::.\textit{:role/(.})", input.identity.arn)
    role := split(input.identity.arn, "/")\cite{ref1}
    user := input.session.verified\_email
}

\# Normalize GCP Service Account
identity = {"provider": "gcp", "user": sa, "role": "service\_account"} {
