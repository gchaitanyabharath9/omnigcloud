...
\textbf{Metric:} \texttt{http\textit{request}duration\_seconds}
...
\item Daily storage: 1.2510^12  86,400  16 = 1.7 PB/day
...
\begin{itemize}
\item tenant\textit{id, user}id, device\textit{type, session}id
...
\texttt{`}
http\textit{requests}total{method="GET", status="200"} = 1,234,567
\texttt{`}

\textbf{Gauge:} Point-in-time value (current queue depth)
\texttt{`}
queue\_depth{service="order-processor"} = 42
\texttt{`}

\textbf{Histogram:} Distribution of values (latency percentiles)
\texttt{`}
http\textit{request}duration\textit{seconds}bucket{le="0.1"} = 950
http\textit{request}duration\textit{seconds}bucket{le="0.5"} = 990
http\textit{request}duration\textit{seconds}bucket{le="1.0"} = 998
\texttt{`}

\textbf{Summary:} Pre-calculated percentiles (client-side)
\texttt{`}
http\textit{request}duration\_seconds{quantile="0.5"} = 0.12
http\textit{request}duration\_seconds{quantile="0.9"} = 0.45
http\textit{request}duration\_seconds{quantile="0.99"} = 0.98
\texttt{`}
...
\end{itemize}
    type: always\_sample
    condition: status\_code >= 500
\begin{itemize}
\item name: slow\_requests
\end{itemize}
    type: always\_sample
    condition: duration > 2000ms
\begin{itemize}
\item name: specific\_endpoints
\end{itemize}
    type: always\_sample
...
\end{itemize}
    type: probabilistic
    condition: status\_code < 400 AND duration < 1000ms
    sample\_rate: 0.01  \# 1\%
\texttt{`}
...
processors:
  tail\_sampling:
    decision\_wait: 30s
    num\_traces: 100000
    expected\textit{new}traces\textit{per}sec: 10000
    policies:
\begin{itemize}
\item name: error-policy
\end{itemize}
        type: status\_code
        status\textit{code: {status}codes: [ERROR]}
\begin{itemize}
\item name: latency-policy
\end{itemize}
        type: latency
        latency: {threshold\_ms: 2000}
\begin{itemize}
\item name: probabilistic-policy
\end{itemize}
        type: probabilistic
        probabilistic: {sampling\_percentage: 1}
\texttt{`}
...
$$ P\textit{{sample}(t) = \begin{cases} 1 \\& \text{if } t \in \text{Errors} \\ 1 \\& \text{if } latency(t) > T}{p99} \\ 1 \\& \text{if } t \in \text{GoldenSignals} \\ R\_{base} \\& \text{otherwise} \end{cases} $$
...
The total cost of observability \$C\_{total}\$ is defined as:

$$ C\textit{{total} = C}{ingest} + C\textit{{storage} + C}{compute} $$

Substituting the sampling probability:

$$ C\textit{{total} \approx V}{vol} \times [ R\textit{{base} \times (1 - P}{anomaly}) + 1 \times P\textit{{anomaly} ] \times S}{trace} $$

This derivation proves that as system volume \$V\textit{{vol}$ increases linearly, the cost can be capped $O(1)$ by dynamically adjusting \$R}{base}$ inverse to volume, provided that \$P\_{anomaly}\$ remains low.
...
processors:
  tail\_sampling:
    decision\_wait: 30s
    num\_traces: 50000
    expected\textit{new}traces\textit{per}sec: 5000
    policies:
