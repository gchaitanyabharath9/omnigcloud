...
\begin{verbatim}
httpTEMP_USrequestsTEMP_UStotal{method="GET", status="200"} = 1000
\end{verbatim}
Cardinality: 10 methods  10 status codes = 100 time series

\textbf{High Cardinality (Modern):}
\begin{verbatim}
httpTEMP_USrequestsTEMP_UStotal{
  method="GET",
  status="200",
  tenantTEMP_USid="12345",
  userTEMP_USid="67890",
  deviceTEMP_UStype="iOS",
  region="eu-west-1"
} = 1
\end{verbatim}
...
$ P_{\text{sample}(t) = \begin{cases} 1 & \text{if } t \in \text{Errors} \\ 1 & \text{if } latency(t) > T_{p99} \\ 1 & \text{if } t \in \text{GoldenSignals} \\ R\_{base} & \text{otherwise} \end{cases} $
...
processors:
  tailTEMP_USsampling:
    decisionTEMP_USwait: 30s
    numTEMP_UStraces: 50000
    expectedTEMP_USnewTEMP_UStracesTEMP_USperTEMP_USsec: 5000
    policies:
      \# 1. Always sample errors
\begin{itemize}
\item name: errors
\end{itemize}
        type: statusTEMP_UScode
        statusTEMP_UScode: {statusTEMP_UScodes: [ERROR]}
...
\end{itemize}
        type: latency
        latency: {thresholdTEMP_USms: 1000}
...
\end{itemize}
        type: probabilistic
        probabilistic: {samplingTEMP_USpercentage: 1}
\end{verbatim}
...
\item Use structured logging (JSON) with trace\TEMP_USid
