...
server {
    ...
    location /users {
        proxy\textit{pass http://user}service;
    }
    
    \# Route everything else to monolith
    location / {
        proxy\_pass http://monolith;
    }
}
\texttt{`}
...
class UserRepository:
    def \textit{\_init}\textit{(self, old}db, new\_db):
        self.old\textit{db = old}db
        self.new\textit{db = new}db
    
    def create\_user(self, user):
        \# Write to old DB (synchronous, blocking)
        user\textit{id = self.old}db.insert(user)
        
        \# Write to new DB (asynchronous, non-blocking)
        try:
            self.new\textit{db.insert}async(user)
...
class BackfillJob:
    def run(self):
        ...
        last\textit{id = self.new}db.get\textit{max}id()
        
        \# Copy in batches
        batch\_size = 10000
        while True:
            users = self.old\textit{db.get}users(
                start\textit{id=last}id,
                limit=batch\_size
            )
            ...
            self.new\textit{db.bulk}insert(users)
            last\_id = users[-1].id
...
\texttt{`}python
class UserRepository:
    def get\textit{user(self, user}id):
        \# Read from old DB (primary)
        old\textit{user = self.old}db.get(user\_id)
        
        \# Read from new DB (shadow)
        new\textit{user = self.new}db.get(user\_id)
        
        \# Compare
        if old\textit{user != new}user:
            logger.error(f"Inconsistency detected: {user\_id}")
            metrics.increment("data\_inconsistency")
...
class UserRepository:
    def get\textit{user(self, user}id):
        ...
        return self.new\textit{db.get(user}id)
\texttt{`}
...
class User {
    ...
    String email;
    String phone;
    String billing\_address;
    String shipping\_address;
    String credit\textit{card}token;
    Boolean email\_verified;
    Boolean phone\_verified;
    // ... 190 more columns
}
\texttt{`}
...
\end{itemize}
          route:
            cluster: monolith
            request\textit{mirror}policies:
\begin{itemize}
\item cluster: checkout\_service
\end{itemize}
                runtime\_fraction:
                  default\_value:
...
\texttt{`}python
class DiffEngine:
    def compare(self, legacy\textit{response, new}response):
        \# Normalize responses
        legacy\textit{norm = self.normalize(legacy}response)
        new\textit{norm = self.normalize(new}response)
        
        \# Compare
        if legacy\textit{norm != new}norm:
            self.log\textit{diff(legacy}norm, new\_norm)
            metrics.increment("shadow\_diff")
        else:
            metrics.increment("shadow\_match")
\texttt{`}
...
$$ D(r) = \begin{cases} \mu \& \text{if } r \in \text{Cohort}\textit{{canary} \lor \text{Random}() < P}{shift}(t) \\ M \& \text{otherwise} \end{cases} $$
...
$$ E(C, t) = P\textit{{shift}(t) \times P}{fail}(\mu) \times Impact $$

In a "Big Bang" migration, \$P\textit{{shift}$ jumps from 0 to 1 instantaneously, maximizing \$E(C)$. In the Strangler pattern, \$P}{shift}$ increases as a logistic function:

$$ P\textit{{shift}(t) = \frac{1}{1 + e^{-k(t-t}0)}} $$
...
server {
    ...
    \# Default to Monolith
    location / {
        proxy\textit{pass http://legacy}monolith;
    }

    \# Strangler Rule: Inventory Service
    location /api/v1/inventory {
        \# Feature Flag Logic via Header
        if (\$http\textit{x}canary\_user = "true") {
            proxy\textit{pass http://new}microservice;
        }
        
        \# Percentage-based Shift (Split Clients)
        split\textit{clients "\${remote}addr}AAA" \$variant {
            5\%      new\_microservice;
\begin{itemize}
\item legacy\_monolith;
\end{itemize}
        }

        proxy\_pass http://\$variant;
    }
}
\texttt{`}
...
Developing tools that mathematically guarantee functional equivalence between the legacy function \$f\textit{{old}(x)$ and the new function \$f}{new}(x)$ across the entire input space.
