    ...
    return (
        <header className="mb-12 border-b border-[var(--card-border)] pb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm opacity-60 font-mono">
                <div>
                    <span className="block text-[10px] uppercase tracking-widest mb-1">{t('version.label')}</span>
                    <span className="text-white">{t('version.value')}</span>
                </div>
                <div>
                    <span className="block text-[10px] uppercase tracking-widest mb-1">{t('date.label')}</span>
                    <span className="text-white">{t('date.value')}</span>
                </div>
                <div>
                    <span className="block text-[10px] uppercase tracking-widest mb-1">{t('field.label')}</span>
                    <span className="text-white">{t('field.value')}</span>
                </div>
                <div>
                    <span className="block text-[10px] uppercase tracking-widest mb-1">{t('author.label')}</span>
