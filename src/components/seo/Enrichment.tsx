    ...
    <div className="mt-16 p-8 rounded-3xl bg-gradient-to-br from-primary/10 to-transparent border border-primary/20 backdrop-blur-sm relative overflow-hidden group">
      ...
      <div className="relative z-10">
        <h3 className="text-2xl font-black mb-2">
          {tSafe(t, "title", "Ready to regain control?")}
        </h3>
        <p className="text-muted-foreground mb-8 max-w-xl">
          {tSafe(
            t,
            "subtitle",
            "Schedule a comprehensive architecture review with our sovereign cloud experts."
          )}
        </p>
    ...
    <div className="mt-8 mb-12 max-w-3xl" style={{ minHeight: "100px" }}>
      <p className="text-xl leading-relaxed text-muted-foreground font-medium border-l-4 border-primary pl-6 py-2 bg-white/5 rounded-r-xl">
        {tSafe(
          t,
          "AboveTheFold",
          "Empowering organizations to maintain data sovereignty and operational resilience across a global cloud-agnostic fabric."
        )}
      </p>
    </div>
  );
      ...
      <PageShell>
        <div className="mb-8">
          ...
          <p className="text-base text-muted-foreground max-w-2xl opacity-80">
            {tSafe(
              tEnrichment,
              "howItWorks",
              "Our streamlined onboarding process ensures rapid deployment of sovereign infrastructure."
            )}
          </p>
        </div>
        ...
        <div className={`grid gap-6 grid-cols-1 md:grid-cols-3`}>
          {steps.map((i) => (
            <div
              key={i}
              className="glass-panel p-6 md:p-8 rounded-[2rem] relative overflow-hidden group hover:border-primary/50 transition-all duration-300 h-full"
            >
                ...
                <div className="flex-1 w-full">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed italic">
                    {tSafe(
                      t,
                      `steps.${i}.desc`,
                      "Optimizing your infrastructure for global compliance and resilience."
                    )}
                  </p>
                </div>
              </div>
      ...
      <PageShell>
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="relative aspect-video rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl">
            <Image src={imageUrl} alt={alt} fill className="object-cover opacity-80" unoptimized />
            ...
            <div className="absolute bottom-6 left-6 right-6 p-4 backdrop-blur-md bg-black/40 rounded-xl border border-white/10">
              <p className="text-xs font-mono text-primary flex items-center gap-2">
                <Globe size={12} />{" "}
                {tSafe(tEnrichment, "visualSection.tag", "Architecture Overview")}
              </p>
            </div>
          </div>
          <div>
            <h2 className="text-2xl font-black mb-4">
              {tSafe(tEnrichment, "visualSection.title", "System Visualizer")}
            </h2>
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20">
              ...
              <p className="text-xs italic opacity-80">
                {tSafe(
                  tEnrichment,
                  "visualSection.note",
                  "This diagram represents the actual orchestration fabric across multiple cloud regions."
                )}
              </p>
            </div>
          </div>
      ...
      <PageShell>
        <div className="max-w-4xl">
          <h2 className="text-2xl font-black mb-6 flex items-center gap-3">
            ...
            {tSafe(t, "title", "Architectural Deep Dive")}
          </h2>
          <div className="prose prose-invert prose-lg max-w-none mb-12">
            <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">
              {tSafe(
                t,
                "content",
                "Explore the formal methodologies and technical specifications that underpin our sovereign orchestration engine."
              )}
            </p>
          </div>
          ...
          {relatedLinks && (
            <div className="mt-16 pt-16 border-t border-white/5">
              <h4 className="text-sm font-black uppercase tracking-widest text-primary mb-8">
                {tSafe(tEnrichment, "relatedLinks.title", "Related Strategy & Insights")}
              </h4>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {relatedLinks.map((link, i) => (
                  <Link
                    ...
                    className="glass-panel p-6 rounded-2xl hover:border-primary/50 transition-all flex justify-between items-center group"
                  >
    ...
    <Section className="snap-section py-16 bg-gradient-to-b from-transparent to-primary/5">
      <PageShell>
        <div className="max-w-3xl">
          <h2 className="text-2xl font-black mb-6 uppercase tracking-tight text-primary">
            {tSafe(t, "title", "Why Sovereignty Matters")}
          </h2>
          <p className="text-xl text-muted-foreground leading-relaxed font-medium">
            {tSafe(
              t,
              "content",
              "In a fragmenting regulatory landscape, the ability to maintain absolute control over infrastructure is a strategic imperative."
            )}
          </p>
        </div>
      </PageShell>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div>
              <h2 className="text-3xl font-black mb-6 tracking-tight">
                {tSafe(t, "title", "Technical Insights")}
              </h2>
              <p className="text-lg text-muted-foreground leading-relaxed whitespace-pre-wrap max-w-4xl">
                {tSafe(
                  t,
                  "content",
                  "Our methodology leverages automated fabric kernels to enforce policy as logic, ensuring consistent behavior across heterogeneous providers."
                )}
              </p>
            </div>
          </div>
            ...
            {count.map((i) => (
              <div key={i} className="border-b border-white/5 pb-8">
                <h3 className="text-xl font-bold mb-4 text-foreground">
                  {tSafe(t, `items.${i}.q`, "Frequently Asked Question")}
                </h3>
                <p className="text-muted-foreground leading-relaxed leading-7">
                  {tSafe(
                    t,
                    `items.${i}.a`,
                    "Response details currently being synchronized from our technical documentation repository."
                  )}
                </p>
              </div>
            ))}
