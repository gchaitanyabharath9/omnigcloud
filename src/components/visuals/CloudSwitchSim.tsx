      ...
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-[10px] opacity-60 font-mono tracking-widest text-primary">
              Protocol: ACC-Migrate-v7
            </p>
          </div>
          <div className="flex flex-col items-end">
            <span
              className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-widest ${status === "transferring" ? "bg-orange-500 animate-pulse" : status === "success" ? "bg-green-500" : "bg-white/10"}`}
            >
              {status === "idle"
                ? "Ready"
                : status === "transferring"
        ...
        <div className="grid grid-cols-7 items-center gap-4 mb-8">
          ...
          <div className="col-span-3 flex flex-col items-center gap-4 relative">
            <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden relative">
              ...
              {status === "transferring" && (
                <div
                  className="absolute top-0 h-full w-20 bg-white shadow-[0_0_20px_white] blur-sm animate-[shimmer_1.5s_infinite]"
          ...
          {/* Target Node */}
          <div
            className={`col-span-2 p-4 glass-panel border transition-all duration-1000 rounded-xl flex flex-col items-center gap-3 ${status === "success" ? "border-primary bg-primary/20 scale-105 shadow-[0_0_30px_rgba(59,130,246,0.2)]" : "border-white/10 bg-white/5"}`}
          >
            <div
              className={`p-3 rounded-full border transition-all duration-1000 ${status === "success" ? "bg-primary/30 border-primary animate-pulse" : "bg-white/10 border-white/10"}`}
            >
            ...
            <div className="flex gap-1">
              <div
                className={`w-1.5 h-1.5 rounded-full ${status === "success" ? "bg-green-500" : "bg-white/10"}`}
              ></div>
              <div
                className={`w-1.5 h-1.5 rounded-full ${status === "success" ? "bg-green-500" : "bg-white/10"}`}
              ></div>
            ...
            <div key={i} className="flex items-center gap-3 opacity-100">
              ...
              <div>
                ...
                <div className="text-[10px] font-bold">
                  {status === "transferring" ? "..." : item.value}
                </div>
              </div>
            </div>
          ))}
