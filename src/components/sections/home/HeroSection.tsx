...
export default function HeroSection() {
  ...
  return (
    <section
      id="hero"
      className="snap-section relative min-h-screen flex flex-col justify-start pb-8 overflow-hidden"
    >
        ...
        <div className="hero-grid-layout">
          ...
          <div className="flex flex-col gap-8">
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <Link
                  ...
                  className="btn-secondary px-7 py-3 text-base flex items-center gap-2"
                >
                  <Layers size={18} /> {t("ctaArchitecture")}
                </Link>
                <Link
                  href="/docs/whitepaper"
                  className="btn-secondary px-7 py-3 text-base flex items-center gap-2 opacity-80"
                >
            ...
            <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-8 backdrop-blur-md relative overflow-hidden">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-sm font-extrabold uppercase text-primary mb-2 tracking-widest">
                    {t("monitor.title")}
                  </h3>
              ...
              <div className="h-[100px] relative my-4">
                <svg width="100%" height="100" viewBox="0 0 400 100" preserveAspectRatio="none">
                  <path
                    d="M0 60 Q 40 50, 80 55 T 160 40 T 240 50 T 320 35 T 400 30"
                    fill="none"
                    stroke="var(--primary)"
                    strokeWidth="4"
                  />
              ...
              <p className="text-sm text-muted leading-relaxed">{t("monitor.desc")}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-8">
            <div className="glass-panel p-0 overflow-hidden bg-surface-2 shadow-2xl">
              <div className="p-8 flex flex-col gap-6">
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { l: t("stats.assets"), v: "$2.4B" },
                    { l: t("stats.drift"), v: "0%", c: "#10b981" },
                    { l: t("stats.nodes"), v: "4k+", c: "var(--primary)" },
                  ].map((s, i) => (
                    <div key={i} className="p-4 bg-white/5 rounded-xl border border-card-border">
                      ...
                      <div
                        ...
                        style={{ color: s.c || "var(--foreground)" }}
                      >
                        {s.v}
                      </div>
                    </div>
            ...
            <div className="glass-panel p-8 bg-blue-500/5">
              <h3 className="text-sm font-black uppercase mb-6 tracking-widest">
                {t("compliance.title")}
              </h3>
                  ...
                  <div key={idx} className="flex flex-col gap-2">
                    ...
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${reg.val}%`, background: reg.color }}
                      ></div>
                    </div>
                  </div>
                ))}
              </div>
              <p className="text-xs text-muted mt-6">{t("compliance.desc")}</p>
            </div>
          </div>
        </div>
      </Container>
      ...
      <div className="w-full border-t border-card-border bg-slate-950/40 backdrop-blur-md py-6 mt-auto">
        <Container className="flex flex-col items-center gap-4">
          <div className="text-[0.65rem] font-black text-muted tracking-[0.2em] uppercase">
            {t("trust")}
          </div>
