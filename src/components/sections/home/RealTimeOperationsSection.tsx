        ...
        <section id="operations" className="snap-section" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', background: 'var(--background)' }}>
            <div className="container">
                <div style={{ marginBottom: '3rem', textAlign: 'center' }}>
                    <h2 style={{ fontSize: '2.5rem', fontWeight: 900, marginBottom: '1rem' }} className="text-gradient">{t('title')}</h2>
                ...
                <div className="grid-2" style={{ gap: '2rem' }}>
                    ...
                    <div className="glass-panel" style={{ padding: '2rem', borderRadius: '1.5rem', border: '1px solid var(--card-border)', background: 'var(--bg-surface-2)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
                            ...
                            <h3 style={{ fontSize: '1.25rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{t('liveMetrics.title')}</h3>
                        </div>
                        ...
                        <div style={{
                            ...
                            {[0, 1, 2, 3].map(i => (
                                <div key={i} style={{
                                    ...
                                    <div style={{ fontSize: '0.8rem', opacity: 0.6, marginBottom: '0.5rem' }}>{t(`liveMetrics.items.${i}.label`)}</div>
                                    <div style={{ fontSize: '2.5rem', fontWeight: 900, color: t(`liveMetrics.items.${i}.color`) }}>{t(`liveMetrics.items.${i}.value`)}</div>
                                </div>
                            ))}
                        </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '1rem' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>{t('networkStatus.items.0.title')}</span>
                                        <span style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{t('networkStatus.items.0.desc')}</span>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '1rem' }}>
                                    <CheckCircle size={20} color="#10b981" />
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>{t('networkStatus.items.1.title')}</span>
                                        <span style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{t('networkStatus.items.1.desc')}</span>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '1rem' }}>
                                    <Activity size={20} color="#f59e0b" />
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>{t('networkStatus.items.2.title')}</span>
                                        <span style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{t('networkStatus.items.2.desc')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                        {[
                                            ...
                                            <div key={i} style={{ position: 'relative', width: '48px', height: '48px', borderRadius: '50%', border: '3px solid var(--bg-surface-2)', marginLeft: i > 0 ? '-16px' : 0, overflow: 'hidden', boxShadow: '0 4px 10px rgba(0,0,0,0.3)' }}>
                                                <Image src={src} alt={t('expertCallout.expertAlt')} fill style={{ objectFit: 'cover' }} unoptimized />
                                            </div>
                                        ))}
                                    </div>
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontSize: '1.1rem', fontWeight: 900, lineHeight: 1 }}>{t('expertCallout.count')}</span>
                                        <span style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>{t('expertCallout.readyDesc')}</span>
                                    </div>
                                </div>
