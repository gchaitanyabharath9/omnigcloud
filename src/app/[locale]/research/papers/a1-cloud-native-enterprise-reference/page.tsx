...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: 'Papers.A1' });
            ...
            <header className="relative py-24 overflow-hidden border-b border-white/5">
                ...
                <div className="container relative z-10 mx-auto px-4">
                    ...
                    <div className="flex flex-wrap items-center gap-8 text-sm font-mono text-slate-400">
                        ...
                        <div className="flex items-center gap-2">
                            <FileText size={16} className="text-primary" /> {t('meta.pages')}
                        </div>
                    </div>
                </div>
            </header>
                ...
                <div className="max-w-4xl mx-auto">
                    <div className="p-8 rounded-2xl bg-white/5 border border-white/10 mb-12">
                        ...
                        <Link
                            ...
                            className="flex items-center justify-center gap-2 w-full py-3 rounded-lg bg-primary/10 border border-primary/20 text-primary font-bold text-sm uppercase tracking-wider hover:bg-primary/20 transition-all"
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="p-6 rounded-xl bg-white/5 border border-white/10">
                            <div className="text-xs text-slate-500 uppercase tracking-wider mb-2">{t('details.classification.label')}</div>
                            <div className="text-primary font-bold">{t('details.classification.value')}</div>
                        </div>
                        <div className="p-6 rounded-xl bg-white/5 border border-white/10">
                            <div className="text-xs text-slate-500 uppercase tracking-wider mb-2">{t('details.security.label')}</div>
                            <div className="text-primary font-bold">{t('details.security.value')}</div>
                        </div>
                        <div className="p-6 rounded-xl bg-white/5 border border-white/10">
                            <div className="text-xs text-slate-500 uppercase tracking-wider mb-2">{t('details.status.label')}</div>
                            <div className="text-green-500 font-bold">{t('details.status.value')}</div>
                        </div>
                    </div>
                </div>
            </main>
