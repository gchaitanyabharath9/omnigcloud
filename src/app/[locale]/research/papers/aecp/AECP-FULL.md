...
- **A1** defines the _structure_ (Separation of Planes).
- **AECP** defines the _mechanism_ (The Control Plane itself).
- **A6** defines the _logic_ (Adaptive Policy).
...
Traditional enterprise architecture treats governance as an overlay—a set of rules applied _after_ infrastructure is provisioned. This isn't just inefficient. It's architecturally wrong. AECP inverts this model, enforcing a strict separation of concerns where the **Control Plane** (Policy) operates asynchronously from the **Data Plane** (Infrastructure), bound only by late-binding enforcement agents.
