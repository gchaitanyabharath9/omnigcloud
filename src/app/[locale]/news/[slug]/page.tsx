      ...
      <div className="relative h-[60vh] min-h-[400px] w-full">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16">
          <div className="container mx-auto max-w-4xl">
            ...
            <p className="text-xl text-slate-300 font-medium">{source}</p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-3xl px-6 py-16">
        <div className="prose prose-lg prose-invert prose-blue max-w-none">
          <p className="lead text-2xl text-slate-200 mb-10 leading-relaxed font-light">
            {t(`news.${slug}.desc` as any) ||
              "Detailed analysis of this sovereign cloud breakthrough."}
          </p>
          ...
          <p>{t("article.solutionDesc")}</p>

          <div className="bg-slate-900 border-l-4 border-blue-500 p-6 my-8 rounded-r-xl">
            <p className="italic text-slate-300 m-0">"{t("article.quote")}"</p>
            <div className="mt-4 font-bold text-white">{t("article.quoteAuthor")}</div>
          </div>

          <h3>{t("article.impactTitle")}</h3>
          <p>{t("article.impactDesc")}</p>
          <p>
            {t("article.docsLinkPrefix")} <Link href="/docs">{t("article.docsLinkText")}</Link>
            {t("article.docsLinkSuffix")}
          </p>
        </div>
