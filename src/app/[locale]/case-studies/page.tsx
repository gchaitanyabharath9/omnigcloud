        ...
        <div className="snap-container">
            ...
            <section id="hero" className="snap-section container">
                <div style={{ position: 'relative', borderRadius: '2rem', overflow: 'hidden', height: '400px', border: '1px solid var(--card-border)', background: 'var(--card-bg)' }}>
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: 'radial-gradient(circle, var(--primary-glow) 0%, transparent 100%)', padding: '2rem', textAlign: 'center' }}>
                        ...
                        <h1 style={{ fontSize: '3.5rem', fontWeight: 950, marginBottom: '1rem', lineHeight: 1, letterSpacing: '-1.5px', color: 'var(--foreground)' }}>{t("hero.title")}</h1>
                        <p style={{ fontSize: '1.2rem', color: 'var(--foreground)', opacity: 0.6, maxWidth: '800px', lineHeight: 1.5 }} dangerouslySetInnerHTML={{ __html: t.raw("hero.subtitle").replace('multi-billion dollar', '<span style="color: var(--primary); fontWeight: 800">multi-billion dollar</span>') }}>
                        </p>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '3rem', height: '100%', maxHeight: 'calc(100vh - 140px)' }}>
                            ...
                            <div className="glass-panel" style={{ padding: '2.5rem', flex: 1, borderRadius: '2.5rem' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                                    <div style={{ padding: '1.5rem', background: 'rgba(0,0,0,0.2)', borderRadius: '1.5rem', border: '1px solid rgba(255,255,255,0.05)' }}>
                                        <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', fontWeight: 800, marginBottom: '0.5rem' }}>{t("labels.challenge")}</div>
                                    ...
                                    <div style={{ padding: '1.5rem', background: 'rgba(96, 239, 255, 0.05)', borderRadius: '1.5rem', border: '1px solid rgba(96, 239, 255, 0.2)' }}>
                                        <div style={{ color: '#60efff', fontSize: '0.75rem', fontWeight: 800, marginBottom: '0.5rem' }}>{t("labels.solution")}</div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            <div className="glass-panel" style={{ padding: '3rem', borderRadius: '3rem', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                <h4 style={{ color: 'white', fontWeight: 800, textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '0.1rem', marginBottom: '3rem' }}>{t("labels.validated")}</h4>
                            ...
                            <div className="glass-panel" style={{ padding: '2rem', borderRadius: '2rem', background: 'rgba(16, 185, 129, 0.05)', border: '1px solid rgba(16, 185, 129, 0.2)', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <div style={{ textAlign: 'center' }}>
                                    ...
                                    <h4 style={{ color: 'white', fontWeight: 800, marginBottom: '1rem' }}>{t("labels.whitepaper")}</h4>
                                    <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', marginBottom: '1.5rem' }}>{t("labels.whitepaperDesc")}</p>
                                    <Link href="/contact" className="btn-primary" style={{ width: '100%', borderRadius: '1rem', background: '#10b981', borderColor: '#10b981' }}>{t("labels.request")}</Link>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <section id="sitemap" className="snap-section" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#020617' }}>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ textAlign: 'center' }}>
                        <div style={{ color: '#60efff', fontWeight: 800, fontSize: '0.8rem', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '1rem' }}>{t("labels.sitemapTitle")}</div>
                        <h2 style={{ color: 'white', fontSize: '2.5rem', fontWeight: 900 }}>{t("labels.sitemapDir").split(' ')[0]} <span style={{ color: 'rgba(255,255,255,0.3)' }}>{t("labels.sitemapDir").split(' ').slice(1).join(' ')}</span></h2>
                    </div>
                </div>
                <Footer />
            </section>
