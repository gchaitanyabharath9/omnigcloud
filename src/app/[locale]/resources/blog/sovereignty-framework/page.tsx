...
export async function generateMetadata({
  ...
  return {
    ...
    description:
      "A formal architectural framework for evaluating and enforcing digital sovereignty in multi-cloud and hybrid environments.",
    keywords: [
...
export default async function SovereigntyFrameworkPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
        ...
        <PageShell>
          <div className="max-w-3xl mx-auto">
            ...
            <div className="prose prose-invert prose-lg max-w-none leading-relaxed">
              ...
              <p className="mb-6">{t("body.exitGateText")}</p>

              <h3 className="text-2xl font-bold mt-12 mb-4">{t("body.mathTitle")}</h3>
              <div
                className="bg-black/50 p-8 rounded-3xl font-mono text-sm border border-primary/20 my-8"
                dangerouslySetInnerHTML={{ __html: t.raw("body.mathBody") }}
              />

              <h2 className="text-3xl font-bold mt-16 mb-8">{t("body.nodesTitle")}</h2>
              <p className="mb-6">{t("body.nodesText")}</p>

              <h3 className="text-2xl font-bold mt-12 mb-4">{t("body.frameworkStepsTitle")}</h3>
              <ul className="space-y-4 mb-12">
                {[0, 1, 2].map((i) => (
                  <li key={i} className="flex gap-4">
                    <div className="shrink-0 w-6 h-6 rounded-full bg-primary flex items-center justify-center text-[10px] font-bold">
                      {i + 1}
                    </div>
