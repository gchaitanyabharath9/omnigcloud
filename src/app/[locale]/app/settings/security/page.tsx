...
export default async function SecuritySettingsPage() {
  ...
  const maskId = (id: string | undefined) => {
    if (!id || id === "your_google_client_id" || id === "your_entra_id_client_id")
      return "• • • • • • • •";
  ...
  const providers = [
    {
      ...
      enabled:
        !!process.env.AUTH_GOOGLE_ID && process.env.AUTH_GOOGLE_ID !== "your_google_client_id",
    },
    {
      ...
      enabled:
        !!process.env.AUTH_ENTRA_ID && process.env.AUTH_ENTRA_ID !== "your_entra_id_client_id",
    },
    {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4">
          {providers.map((p) => (
            <div
              key={p.id}
              className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-xl shadow-sm space-y-4"
            >
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-lg">{p.name}</h3>
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${p.enabled ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-500"}`}
                >
      ...
      <section className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 p-6 rounded-xl space-y-4">
        ...
        <p className="text-sm text-blue-800 dark:text-blue-300">{t("advancedDescription")}</p>
