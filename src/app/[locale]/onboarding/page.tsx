        ...
        <div className="animate-fade-in" style={{ paddingBottom: '4rem' }}>
            ...
            <section className="container" style={{ padding: '0.5rem 0 2rem', textAlign: 'center', position: 'relative' }}>
                <div style={{ marginTop: '1rem', position: 'relative', borderRadius: '1.5rem', overflow: 'hidden', padding: '3rem 2rem', background: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <h1 style={{ fontSize: '3rem', fontWeight: 950, marginBottom: '0.5rem', letterSpacing: '-1.5px', color: 'var(--foreground)' }}>{t('hero.title')}</h1>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <div className="glass-panel" style={{ padding: '0', borderRadius: '2rem', overflow: 'hidden', height: '300px', position: 'relative' }}>
                            ...
                            <div style={{ position: 'absolute', bottom: '2rem', left: '2rem' }}>
                                <div style={{ fontWeight: 800, fontSize: '1.25rem' }}>{t('empowered.title')}</div>
                                <div style={{ opacity: 0.8 }}>{t('empowered.subtitle')}</div>
                            </div>
                        </div>

                        <div className="glass-panel" style={{ padding: '3.5rem', borderRadius: '2.5rem', border: '1px solid rgba(255,255,255,0.1)' }}>
                            <h3 style={{ fontSize: '1.8rem', color: 'white', marginBottom: '2rem', textAlign: 'center' }}>{t('form.title')}</h3>
                            <form style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                    <label style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem' }}>{t('form.cloudProvider.label')}</label>
                                    <select style={{ background: '#0a2540', border: '1px solid rgba(255,255,255,0.2)', padding: '0.75rem 1rem', borderRadius: '0.5rem', color: 'white' }}>
                                        <option>{t('form.cloudProvider.options.aws')}</option>
                                        <option>{t('form.cloudProvider.options.azure')}</option>
                                        <option>{t('form.cloudProvider.options.gcp')}</option>
                                        <option>{t('form.cloudProvider.options.hybrid')}</option>
                                    </select>
                                </div>
                                <button type="submit" className="btn-primary" style={{ padding: '1rem', borderRadius: '0.5rem', fontWeight: 800, marginTop: '1rem' }}>
                                    {t('form.submit')} <ArrowRight size={18} style={{ marginLeft: '0.5rem' }} />
                                </button>
            ...
            <section className="container section-padding border-top" style={{ borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                <div style={{ display: 'flex', justifyContent: 'center', gap: '4rem', opacity: 0.6, flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'white', fontWeight: 700 }}><CheckCircle size={20} color="#60efff" /> {t('trust.soc2')}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'white', fontWeight: 700 }}><Shield size={20} color="#60efff" /> {t('trust.gdpr')}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'white', fontWeight: 700 }}><RefreshCcw size={20} color="#60efff" /> {t('trust.sla')}</div>
                </div>
            </section>
        </div>
    );
