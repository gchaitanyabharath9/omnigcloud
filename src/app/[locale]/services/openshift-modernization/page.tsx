...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: "Metadata.Services.openshift-modernization",
  });
...
export default async function OpenShiftModernizationPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
        ...
        <PageShell>
          <div className="max-w-4xl">
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                href={`/${locale}/contact`}
                className="btn-primary py-4 px-10 rounded-full font-bold bg-red-600 border-red-600 hover:bg-red-700"
              >
                {t("hero.ctaPrimary")}
              </Link>
              <Link
      ...
      <Section className="snap-section py-24 border-y border-white/5 bg-red-900/5">
        <PageShell>
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="text-sm font-black text-red-500 uppercase tracking-widest mb-6">
                {t("core.badge")}
              </h2>
              <h3 className="text-4xl font-bold mb-8">{t("core.title")}</h3>
              <p className="text-lg opacity-80 leading-relaxed mb-6">{t("core.p1")}</p>
              <p
                className="text-lg opacity-80 leading-relaxed"
                dangerouslySetInnerHTML={{ __html: t.raw("core.p2") }}
              />
            </div>
            <div className="grid grid-cols-1 gap-6">
              {[
                ...
                <div
                  key={i}
                  className="glass-panel p-6 rounded-2xl flex gap-6 items-start hover:border-red-500/30 transition-all border-red-500/5"
                >
                  ...
                  <div>
                    <h4 className="font-bold mb-2 text-xl">
                      {t(`core.features.${feature.key}.title`)}
                    </h4>
                    <p className="opacity-60 text-sm leading-relaxed">
                      {t(`core.features.${feature.key}.desc`)}
                    </p>
                  </div>
                </div>
              ))}
        ...
        <PageShell>
          <div className="max-w-3xl mx-auto">
            ...
            <div className="prose prose-invert prose-slate max-w-none leading-relaxed">
              <h4 className="text-xl font-bold mb-4 mt-10 text-red-500">
                {t("factory.item1Title")}
              </h4>
              <p className="mb-6">{t("factory.item1Desc")}</p>
              <h4 className="text-xl font-bold mb-4 text-red-500">{t("factory.item2Title")}</h4>
              <p className="mb-6">{t("factory.item2Desc")}</p>
              <h4 className="text-xl font-bold mb-4 text-red-500">{t("factory.item3Title")}</h4>
              <p>{t("factory.item3Desc")}</p>
            </div>
          </div>
        </PageShell>
      </Section>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-4">
              {[0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="glass-panel p-6 rounded-2xl border-white/5 border-red-500/5 hover:border-red-500/20 transition-colors"
                >
        ...
        <PageShell>
          <div className="grid md:grid-cols-2 gap-8">
            <Link
              ...
              className="glass-panel p-10 rounded-[2.5rem] group hover:border-red-500/50 transition-all border-red-500/10 bg-red-500/5"
            >
              <h4 className="text-2xl font-bold mb-4 group-hover:text-red-500 transition-colors">
                {t("footer.automationTitle")}
              </h4>
            ...
            <Link
              ...
              className="glass-panel p-10 rounded-[2.5rem] group hover:border-white/20 transition-all border-white/5"
            >
