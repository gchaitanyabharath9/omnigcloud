...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: 'Metadata.Services.application-modernization' });
...
export default async function ApplicationModernizationPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
                ...
                <PageShell>
                    <div className="max-w-4xl">
                        ...
                        <div className="flex flex-wrap gap-4">
                            <Link href={`/${locale}/contact`} className="btn-primary py-4 px-10 rounded-full font-bold bg-purple-600 border-purple-600 hover:bg-purple-700">{t('hero.ctaPrimary')}</Link>
            ...
            <Section className="py-24 border-y border-white/5 bg-purple-900/5">
                <PageShell>
                    <div className="grid lg:grid-cols-2 gap-16 items-center">
                        <div>
                            <h2 className="text-xs font-black text-purple-500 uppercase tracking-[0.2em] mb-4">{t('challenge.badge')}</h2>
                        ...
                        <div className="glass-panel p-10 border-purple-500/20 bg-purple-500/5">
                            <div className="text-5xl font-black text-purple-500/10 mb-4">{t('challenge.debt')}</div>
                ...
                <PageShell>
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="space-y-6">
                            {[0, 1, 2].map((i) => (
                                <div key={i} className="glass-panel p-6 border-white/5 hover:border-purple-500/20 transition-colors">
