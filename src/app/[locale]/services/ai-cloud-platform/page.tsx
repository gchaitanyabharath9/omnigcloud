...
export default async function AiCloudPlatformPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
        ...
        <PageShell>
          <div className="max-w-4xl">
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                href={`/${locale}/contact`}
                className="btn-primary py-4 px-10 rounded-full font-bold bg-cyan-600 border-cyan-600 hover:bg-cyan-700"
              >
                {t("hero.ctaPrimary")}
              </Link>
              <Link
      ...
      <Section className="py-24 border-y border-white/5 bg-cyan-900/5">
        <PageShell>
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-xs font-black text-cyan-400 uppercase tracking-[0.2em] mb-4">
                {t("challenge.badge")}
              </h2>
        ...
        <PageShell>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div
                key={i}
                className="glass-panel p-8 relative overflow-hidden group border-cyan-400/10 hover:border-cyan-400/30"
              >
                <span className="absolute top-4 right-4 text-4xl font-black text-cyan-400/5 group-hover:text-cyan-400/10 transition-colors">
                  {item.step}
                </span>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-6">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="glass-panel p-6 border-white/5 hover:border-cyan-400/20 transition-colors"
                >
