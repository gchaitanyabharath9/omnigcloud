                ...
                <PageShell>
                    <div className="max-w-4xl">
                        ...
                        <div className="flex flex-wrap gap-4">
                            ...
                            <Link href={`/${locale}/docs/guide`} className="btn-secondary py-4 px-10 rounded-full font-bold">{t('hero.ctaSecondary')}</Link>
                        </div>
                    </div>
                </PageShell>
            </Section>
            ...
            <Section className="py-24 border-y border-white/5">
                <PageShell>
                    <div className="grid lg:grid-cols-2 gap-16 items-start">
                        <div>
                            <h2 className="text-sm font-black text-primary uppercase tracking-widest mb-6">{t('discovery.badge')}</h2>
                            <h3 className="text-4xl font-bold mb-8">{t('discovery.title')}</h3>
                            <p className="text-lg opacity-80 leading-relaxed mb-6">
                                {t('discovery.p1')}
                            </p>
                            <p className="text-lg opacity-80 leading-relaxed" dangerouslySetInnerHTML={{ __html: t.raw('discovery.p2') }} />
                        </div>
                        <div className="grid grid-cols-1 gap-6">
                            {[
                                ...
                                <div key={i} className="glass-panel p-6 rounded-2xl flex gap-6 items-start hover:border-primary/30 transition-all">
                                    ...
                                    <div>
                                        <h4 className="font-bold mb-2 text-xl">{t(`discovery.features.${feature.key}.title`)}</h4>
                                        <p className="opacity-60 text-sm leading-relaxed">{t(`discovery.features.${feature.key}.desc`)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <PageShell>
                    <div className="grid md:grid-cols-2 gap-8">
                        <Link href={`/${locale}/services/cloud-cost-optimization`} className="glass-panel p-10 rounded-[2.5rem] group hover:border-primary/50 transition-all">
                            <h4 className="text-2xl font-bold mb-4 group-hover:text-primary transition-colors">{t('footer.costTitle')}</h4>
                        ...
                        <Link href={`/${locale}/services/cloud-modernization`} className="glass-panel p-10 rounded-[2.5rem] group hover:border-primary/50 transition-all">
                            <h4 className="text-2xl font-bold mb-4 group-hover:text-primary transition-colors">{t('footer.factoryTitle')}</h4>
