...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: 'Metadata.Services.cloud-cost-optimization' });
...
export default async function CloudCostOptimizationPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
                ...
                <PageShell>
                    <div className="max-w-4xl">
                        ...
                        <div className="flex flex-wrap gap-4">
                            <Link href={`/${locale}/contact`} className="btn-primary py-4 px-10 rounded-full font-bold bg-emerald-600 border-emerald-600 hover:bg-emerald-700">{t('hero.ctaPrimary')}</Link>
            ...
            <Section className="py-24 border-y border-white/5 bg-emerald-900/5">
                <PageShell>
                    <div className="grid lg:grid-cols-2 gap-16 items-start">
                        <div>
                            <h2 className="text-sm font-black text-emerald-500 uppercase tracking-widest mb-6">{t('intelligence.badge')}</h2>
                        ...
                        <div className="grid grid-cols-1 gap-6">
                            {[
                                ...
                                <div key={i} className="glass-panel p-6 rounded-2xl flex gap-6 items-start hover:border-emerald-500/30 transition-all border-emerald-500/5">
                                    <div className="bg-emerald-500/10 p-3 rounded-lg text-emerald-500">
                                        <feature.icon size={24} />
                                    </div>
                                    <div>
                                        <h4 className="font-bold mb-2 text-xl">{t(`intelligence.features.${feature.key}.title`)}</h4>
                                        <p className="opacity-60 text-sm leading-relaxed">{t(`intelligence.features.${feature.key}.desc`)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <PageShell>
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="prose prose-invert prose-slate max-w-none leading-relaxed">
                            <h4 className="text-xl font-bold mb-4 mt-10 text-emerald-500">{t('roadmap.item1Title')}</h4>
                            <p className="mb-6">
                                {t('roadmap.item1Desc')}
                            </p>
                ...
                <PageShell>
                    <div className="max-w-3xl mx-auto">
                        ...
                        <div className="space-y-4">
                            {[0, 1, 2, 3].map((i) => (
                                <div key={i} className="glass-panel p-6 rounded-2xl border-white/5 border-emerald-500/5 hover:border-emerald-500/20 transition-colors">
