...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: 'Metadata.Industries.healthcare' });
...
export default async function HealthcareIndustryPage({ params }: { params: Promise<{ locale: string }> }) {
    const { locale } = await params;
    const t = await getTranslations('Industries.healthcare');
                ...
                <PageShell>
                    <div className="max-w-4xl">
                        <div className="badge badge-primary-subtle mb-6 flex items-center gap-2">
                            <HeartPulse size={14} /> {t('hero.badge')}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-4">
                            ...
                            <Link href={`/${locale}/use-cases/healthcare`} className="btn-secondary py-4 px-10 rounded-full font-bold">{t('hero.ctaSecondary')}</Link>
                        </div>
                    </div>
                </PageShell>
            </Section>
            ...
            <Section className="py-24 border-y border-white/5">
                <PageShell>
                    <div className="grid lg:grid-cols-2 gap-16 items-start">
                        <div>
                            <h2 className="text-sm font-black text-primary uppercase tracking-widest mb-6">{t('paradox.badge')}</h2>
                            <h3 className="text-4xl font-bold mb-8">{t('paradox.title')}</h3>
                            <p className="text-lg opacity-80 leading-relaxed mb-6" dangerouslySetInnerHTML={{ __html: t.raw('paradox.p1') }} />
                            <p className="text-lg opacity-80 leading-relaxed" dangerouslySetInnerHTML={{ __html: t.raw('paradox.p2') }} />
                        </div>
                        <div className="grid grid-cols-1 gap-6">
                            {[
                                { key: 'gating', icon: Lock },
                                { key: 'acceleration', icon: Zap },
                                { key: 'ai', icon: Microchip }
                            ].map((feature, i) => (
                                <div key={i} className="glass-panel p-6 rounded-2xl flex gap-6 items-start hover:border-primary/30 transition-all">
                                    ...
                                    <div>
                                        <h4 className="font-bold mb-2 text-xl">{t(`paradox.features.${feature.key}.title`)}</h4>
                                        <p className="opacity-60 text-sm leading-relaxed">{t(`paradox.features.${feature.key}.desc`)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <PageShell>
                    <div className="grid md:grid-cols-2 gap-8 mb-16">
                        ...
                        <Link href={`/${locale}/services/ai-cloud-platform`} className="glass-panel p-10 rounded-[2.5rem] group hover:border-primary/50 transition-all">
                            <h4 className="text-2xl font-bold mb-4 group-hover:text-primary transition-colors">{t('footer.aiTitle')}</h4>
                            <p className="opacity-60 text-sm mb-6">{t('footer.aiDesc')}</p>
                            <div className="text-primary font-bold flex items-center gap-2">{t('footer.aiLink')} <ArrowRight size={14} /></div>
                        </Link>
                    </div>
