...
export function renderMarkdownToHTML(md: string): string {
  ...
  function renderTable(rows: string[]) {
    ...
    let tableHtml =
      '<div class="overflow-x-auto my-8 border border-white/10 rounded-lg"><table class="w-full text-left text-sm font-mono">';
    ...
    contentRows.forEach((row, idx) => {
      const cells = row
        .split("|")
        .filter((c) => c.trim().length > 0 || row.startsWith("|") || row.endsWith("|"));
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Paragraphs (default)
      ...
      const processedLine = line
        ...
        .replace(
          /`(.*?)`/g,
          '<code class="bg-white/10 px-1.5 py-0.5 rounded text-xs text-primary font-mono">$1</code>'
        );
