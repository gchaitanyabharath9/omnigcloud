...
export async function coreMiddleware(request: NextRequest) {
  ...
  const isGoodBot =
    /googlebot|bingbot|yandexbot|duckduckbot|slurp|baiduspider|facebot|facebookexternalhit|twitterbot|linkedinbot|pinterestbot|applebot|whatsapp|flipboard|tumblr|skypeuripreview|nuzzel|discordbot|google pagead/i.test(
      userAgent
    );

  const isBadBot =
    /gptbot|ccbot|google-extended|anthropic-ai|claude-web|bytespider|omgilibot|facebookbot|diffbot|ia_archiver|cohere-ai|mj12bot/i.test(
      userAgent
    );
