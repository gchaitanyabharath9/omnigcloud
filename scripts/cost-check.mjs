...
function checkCosts() {
    ...
    files.forEach(file => {
        ...
        if (content.includes('getServerSideProps')) {
            errors.push(`❌ SSR Detected in ${path.relative(process.cwd(), file)}: 'getServerSideProps' is forbidden (Cost Risk). Use 'getStaticProps' or Server Components.`);
        }

        // 2. Check for force-dynamic
        if ((content.includes("dynamic = 'force-dynamic'") || content.includes('dynamic = "force-dynamic"')) && !file.includes('metrics\\route.ts') && !file.includes('metrics/route.ts')) {
            errors.push(`❌ Force Dynamic Detected in ${path.relative(process.cwd(), file)}: 'force-dynamic' can lead to excessive function usage.`);
        }
        ...
        if (content.includes('next/image')) {
            ...
            if (imageInvocations) {
                imageInvocations.forEach(usage => {
                    if (!usage.includes('unoptimized') && (usage.includes('http') || usage.includes('src={'))) {
    ...
    if (fs.existsSync(middlewarePath)) {
        ...
        if (mwContent.includes('/_next/static') || mwContent.includes('.png') || mwContent.includes('.jpg')) {
            errors.push(`❌ Middleware Scope Risk: 'proxy.ts' appears to match static assets. Check 'matcher' config.`);
        }
    }
