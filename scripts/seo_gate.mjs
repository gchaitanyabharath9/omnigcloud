...
const REQUIRED_PAPERS = [
  ...
  "/en/research/papers/distributed-systems-resilience",
...
const ALLOWED_NOINDEX_PATHS = ["/dashboard", "/account", "/settings", "/admin"];
  ...
  try {
    ...
    if (!res.ok) {
      violations.push({
        type: "SITEMAP",
        url: "/sitemap.xml",
        message: `Sitemap returned ${res.status}`,
      });
      return;
    }
    ...
    violations.push({
      type: "SITEMAP",
      url: "/sitemap.xml",
      message: `Failed to fetch sitemap: ${e.message}`,
    });
  }
}
    ...
    if (res.status >= 300 && res.status < 400) {
      ...
      if (sitemapUrls.has(url)) {
        violations.push({
          ...
          url,
          message: `Sitemap URL redirects to ${res.headers.get("location")}`,
        });
      }
      return;
    }
    ...
    if (!canonical) {
      ...
      if (!canonical.startsWith("http")) {
        violations.push({
          type: "CANONICAL",
          url,
          message: `Invalid canonical format: ${canonical}`,
        });
      } else {
        try {
          const parsedCanonical = new URL(canonical, BASE_URL);
          if (
            parsedCanonical.hostname === "omnigcloud.com" ||
            parsedCanonical.hostname === "www.omnigcloud.com" ||
            parsedCanonical.origin === new URL(BASE_URL).origin
          ) {
...
async function run() {
  ...
  try {
    ...
    REQUIRED_PAPERS.forEach((p) => {
      ...
      if (!found && sitemapUrls.size > 0) {
        // Only fail if sitemap exists
    ...
    while (queue.length > 0 && processed < MAX_URLS) {
      ...
      if (!visited.has(url)) {
        ...
        process.stdout.write(
          `\rCrawled: ${processed}/${MAX_URLS} | Violations: ${violations.length}`
        );
      }
    }
    console.log("\nCrawl complete.");
