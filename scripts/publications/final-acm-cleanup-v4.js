/**
 * scripts/publications/final-acm-cleanup-v4.js
 */
...
function cleanupTex(content, paperId) {
    ...
    // Match headers like \subsection{Original Contribution (Verified)}
    const headerPattern = /\\(?:sub)*section\{.*(?:Contribution|Innovation|Relationship to A1-A6|Relationship to series).*?\}(?:\\label\{.*?\})?/gi;
    ...
    for (let i = 1; i < sections.length; i++) {
        ...
        if (s.match(/^(?:Original |Verified )?Contribution|Innovation|Relationship to A1-A6|Relationship to series/i)) {
            // Get content until next main section or figure/table
            const body = s.substring(s.indexOf('}') + 1).split(/\\(?:sub)*section\{|\\begin\{figure\}|\\begin\{table\}/)[0];
            extractedContent += body.trim() + "\n\n";
        }
    }

    // Remove the headers globally
    newContent = newContent.replace(headerPattern, '');

    // Scrub the extracted content
    ...
    if (extractedContent.trim()) {
        const introEndMatch = newContent.match(/\\(?:sub)*section\{Introduction\}[\s\S]*?(?=\\section|\\subsection|\\subsubsection)/i);
        if (introEndMatch) {
            newContent = newContent.replace(introEndMatch[0], (m) => m.trim() + '\n\n' + extractedContent.trim() + '\n\n');
        }
    }
...
console.log('--- Final ACM Cleanup (Version 4.0) ---');
let changedCount = 0;
