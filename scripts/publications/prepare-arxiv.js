 ...
 * prepare-arxiv.js
 * 
 * Prepares standalone arXiv submission bundles.
 * - Relativizes paths
 * - Copies assets
 * - Injects self-containment context
 * - Final tone polish
 */
...
const TONE_CHECKS = [
    ...
    // Fix Absolute Paths in metadata/alt text
    { r: /[C-c]:\/Users\/[^\s"\}\]]+/g, s: '' },
    { r: /\/Users\/[^\s"\}\]]+/g, s: '' },
    // Cleanup empty alt if result is empty
    { r: /alt=\{\}/g, s: '' }
];

if (!fs.existsSync(MANIFEST_PATH)) {
    console.error("Manifest not found. Run setup-arxiv.ps1 first.");
    process.exit(1);
}
...
console.log(`Processing ${manifest.length} papers for arXiv submission...`);

manifest.forEach(paper => {
    console.log(`[${paper.id}] Bundling...`);
    ...
    // 2. Read Source TeX
    const sourcePath = path.join(process.cwd(), paper.source_tex);
    ...
    // 6. Comments & Metadata
    const header = `% arXiv submission source for ${paper.id}
...
## Metadata
- **Title**: ${paper.title}
- **Category**: ${paper.primary_arxiv_category || 'cs.AR'}
