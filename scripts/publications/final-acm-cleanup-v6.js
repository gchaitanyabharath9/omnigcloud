/**
 * scripts/publications/final-acm-cleanup-v6.js
 */
...
function cleanupTex(content, paperId) {
    ...
    newContent = newContent.replace(/\\begin\{abstract\}([\s\S]*?)\\end\{abstract\}/, (match, body) => {
        ...
        b = b.replace(/\. This (work demonstrates|paper presents)\b/g, '.');
    ...
    newContent = newContent.replace(/\\(?:sub)*section\{(?:Original |Verified )?Contribution(?:s)?(?:\s*\(Verified\))?\}(?:\\label\{.*?\})?/gi, '');
    newContent = newContent.replace(/\\(?:sub)*section\{Relationships to (?:the )?(?:A1.A6 )?series(?:\(?A1.A6\s*series\)?|)?\}(?:\\label\{.*?\})?/gi, '');
    newContent = newContent.replace(/\\(?:sub)*section\{Why This Innovation Was Needed Now\}(?:\\label\{.*?\})?/gi, '');

    // Scrub self-assertive words
    ...
    newContent = newContent.replace(/AECP executes A1/gi, 'AECP implements the reference architecture');
    newContent = newContent.replace(/\bto (?:the best of )?our knowledge\b/gi, '');

    // Cleanup Duplicated Novelty Content (if any)
    // We'll look for repeated paragraphs that start with "This paper formalizes" or similar
    // Actually, let's just do a greedy deduplication of long paragraphs if possible, or skip for now.
...
console.log('--- Final ACM Cleanup (Version 6.0) ---');
let changedCount = 0;
