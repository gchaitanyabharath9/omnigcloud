 ...
 * prepare-ieee.js
 * 
 * Prepares standalone IEEE submission bundles.
 * - Source: papers/ieee/<ID>-ieee.tex
 * - Output: submission/ieee/<ID>/
...
const SOURCE_ROOT = path.join(process.cwd(), 'papers', 'ieee');
const OUTPUT_ROOT = path.join(process.cwd(), 'submission', 'ieee');
...
const TONE_CHECKS = [
    ...
    // Fix Absolute Paths in metadata/alt text/comments
    { r: /[C-c]:\/Users\/[^\s"\}\]]+/g, s: '' },
    { r: /\/Users\/[^\s"\}\]]+/g, s: '' },
    { r: /alt=\{\}/g, s: '' }
];
...
console.log(`Processing ${manifest.length} papers for IEEE submission...`);

manifest.forEach(paper => {
    console.log(`[${paper.id}] Bundling IEEE...`);
    ...
    // 2. Read Source TeX (IEEE Variant)
    // Filename convention from normalize-papers.js: <ID>-ieee.tex
    const sourceFilename = `${paper.id}-ieee.tex`;
    ...
    content = content.replace(/\\includegraphics(?:\[.*?\])?\{(.*?)\}/g, (match, srcPath) => {
        ...
        if (!path.isAbsolute(srcPath)) {
            absPath = path.resolve(process.cwd(), srcPath); // Usually normalize-papers produced absolute, but fallback
        }
    ...
    // IEEEtran usually puts Abstract, then Keywords, then Introduction.
    // We insert after \section{Introduction}.
    ...
    // 5. Tone Normalization & Cleanup
    TONE_CHECKS.forEach(check => {
        content = content.replace(check.r, check.s);
    });

    // 6. Comments & Metadata
    const header = `% IEEE Conference submission source for ${paper.id}
...
`;
    ...
    // 8. README
    const readme = `# IEEE Submission: ${paper.id}

## Metadata
- **Title**: ${paper.title}
- **Format**: IEEEtran Conference (Double Column)
...
console.log("IEEE Bundling complete.");
