...
test.describe('Responsiveness Gate', () => {
    for (const url of PAGES) {
        for (const viewport of VIEWPORTS) {
            test(`${url} on ${viewport.name} (${viewport.width}x${viewport.height})`, async ({ page }) => {
                ...
                if (consoleErrors.length > 0) {
                    ...
                    console.error(`Recharts sizing errors detected on ${url} @ ${viewport.name}:`, consoleErrors);
                ...
                // Take screenshot
                const safeUrl = url === '/' ? 'home' : url.replace(/\//g, '-').replace(/^-+/, '');
                await page.screenshot({ path: path.join(ARTIFACTS_DIR, `${safeUrl}-${viewport.name}.png`) });
            });
        }
    }
});
