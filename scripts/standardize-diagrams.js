...
papers.forEach((p) => {
  ...
  content = content.replace(/```mermaid([\s\S]*?)```/g, (match, code) => {
    ...
    lines = lines.filter(
      (l) => !l.startsWith("style ") && !l.startsWith("classDef ") && !l.startsWith("class ")
    );
    ...
    bodyContent.forEach((line) => {
      ...
      while ((match = nodeWithLabelRegex.exec(line)) !== null) {
        ...
        if (
          ...
          label.includes("down") ||
          label.includes("degraded") ||
          label.includes("attacker") ||
          ...
          label.includes("shard")
        )
          cls = "State";
        else if (
          label.includes("control") ||
          label.includes("orchestrat") ||
          label.includes("manager") ||
          label.includes("api") ||
          label.includes("gslb") ||
          label.includes("facade") ||
          label.includes("dns") ||
