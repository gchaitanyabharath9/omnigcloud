...
try {
    ...
    REQUIRED_LOCALES.filter(l => l !== 'en').forEach(locale => {
        try {
            const locContent = JSON.parse(fs.readFileSync(path.join(MESSAGES_DIR, `${locale}.json`), 'utf8'));
            ...
            if (diff > enKeys.size * 0.2) {
                console.log(`\x1b[33mWARN: ${locale} key count differs by >20% (${locKeys.size} vs ${enKeys.size})\x1b[0m`);
            }
