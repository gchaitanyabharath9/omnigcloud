...
function log(msg: string) {
  console.log(`[NAV-GATE] ${msg}`);
}
function error(msg: string) {
  console.error(`[NAV-GATE-ERROR] ${msg}`);
}
...
async function validate() {
  ...
  const itemRegex =
    /\{[\s\S]*?id:\s*['"]([^'"]+)['"][\s\S]*?route:\s*['"]([^'"]+)['"](?:[\s\S]*?hash:\s*['"]([^'"]+)['"])?[\s\S]*?\}/g;
      ...
      if (pageFile && fs.existsSync(pageFile)) {
        ...
        if (sectionMatch) {
          ...
          if (!ids.includes(item.hash)) {
            error(
              `MISSING HASH TARGET: ${item.id} -> ${item.route}#${item.hash} (ID not found in page SECTION_IDS)`
            );
            failCount++;
          }
        } else {
          ...
          if (
            !pageContent.includes(`id="${item.hash}"`) &&
            !pageContent.includes(`id={'${item.hash}'}`)
          ) {
            error(
              `MISSING HASH TARGET: ${item.id} -> ${item.route}#${item.hash} (No matching ID found in page content)`
            );
            failCount++;
          }
        }
