    ...
    if (process.env.VERCEL || process.env.CI) {
      ...
      try {
        ...
        if (["all", "smoke"].includes(MODE)) {
          if (!runStepSync("Smoke Tests", "node scripts/smoke-check.mjs", { QUALITY_GATE: "true" }))
            success = false;
        }
        ...
        if (["all", "seo"].includes(MODE)) {
          if (
            !runStepSync("SEO Head Tags Check", "node scripts/seo-sanity.mjs", {
              QUALITY_GATE: "true",
            })
          )
            success = false;
        ...
        if (["all", "responsive", "perf"].includes(MODE)) {
          ...
          if (
            !runStepSync(
              "E2E Quality Gate (Multi-browser/Viewport)",
              "npx playwright test -c playwright.gate.config.ts",
              { QUALITY_GATE: "true" }
            )
          )
            success = false;
