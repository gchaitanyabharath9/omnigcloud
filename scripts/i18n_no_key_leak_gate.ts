...
async function scanForKeyLeaks() {
    ...
    const files = await glob(`${SCAN_DIR}/**/*.{tsx,ts}`, { ignore: ['**/*.d.ts', '**/*.test.ts', '**/*.test.tsx'] });
    ...
    for (const file of files) {
        ...
        for (const pattern of SUSPICIOUS_PATTERNS) {
            ...
            if (matches) {
                console.error(`[FAIL] Potential leaked key found in ${path.relative(process.cwd(), file)}:`);
                matches.forEach(m => console.error(`   ${m.trim()}`));
                hasErrors = true;
            }

            // Also check for "title=" attributes that might contain keys
            const titleAttrRegex = new RegExp(`title=["'][^"']*${pattern.replace('.', '\\.')}[^"']*["']`, 'g');
            ...
            if (titleMatches) {
                ...
                console.error(`[FAIL] Potential leaked key in attribute in ${path.relative(process.cwd(), file)}:`);
