...
const captionExpansions = {
  "a2-high-throughput-distributed-systems/A2-PAPER-FULL.md": [
    {
      target: "Theoretical limit visualized via USL.",
      add: " This visualization clarifies the separation between coordination crosstalk (control path) and throughput scaling (data path) in high-volume systems.",
    },
    {
      target: "The Shock Absorber Architecture.",
      add: " This clarifies the separation between high-velocity ingress (data path) and complex asynchronous processing (control/state reconciliation).",
    },
  ],
  "a3-enterprise-observability-operational-intelligence/A3-PAPER-FULL.md": [
    {
      target: "The Observability Triangle.",
      add: " The model demonstrates the interconnectedness of signals required for rapid correlation across the control and data planes.",
    },
    {
      target: 'The "Cardinality Cliff."',
      add: " This shift in complexity from metrics to traces protects the scalability of the monitoring control plane.",
    },
  ],
  "a4-platform-governance-multicloud-hybrid/A4-PAPER-FULL.md": [
    {
      target: "The Policy-as-Code (PaC) Compilation Pipeline.",
      add: " The diagram illustrates the asynchronous propagation from policy definition (control plane) to runtime enforcement (data path).",
    },
  ],
  "a5-monolith-to-cloud-native-modernization/A5-PAPER-FULL.md": [
    {
      target: "The Strangler Fig Pattern.",
      add: " This shows the incremental transition of the data path from legacy monoliths to cloud-native microservices, managed by a programmable control plane.",
    },
  ],
  "a6-adaptive-policy-enforcement/A6-PAPER-FULL.md": [
    {
      target: "The Autonomic OODA Control Loop.",
      add: " The model defines the deterministic interaction between observability sensors (control plane) and runtime actuators (data path) during steady-state and failure response.",
    },
  ],
  "aecp/AECP-FULL.md": [
    {
      target: "Traditional vs. AECP Architecture.",
      add: " The comparison highlights the inversion of the data path, where policy-defined intent (control plane) strictly governs the infrastructure lifecycle.",
    },
  ],
};
