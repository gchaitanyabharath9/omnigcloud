...
function processSvg(filePath) {
  ...
  for (const [cls, colors] of Object.entries(palette)) {
    // Replace in styles
    const oldStyle =
      `. ${cls}>*{fill:${colors.oldFill}!important;stroke:${colors.oldStroke}!important;stroke-width:2px!important;color:${colors.oldText}!important;}`.replace(
        /\./g,
        `\\.${cls}`
      ); // escape for regex if needed, but we use simple string replace first
    ...
    const regexMain = new RegExp(
      `#my-svg \\.${cls}>\\*\\{fill:${colors.oldFill}!important;stroke:${colors.oldStroke}!important;stroke-width:2px!important;color:${colors.oldText}!important;\\}`,
      "gi"
    );
    ...
    const regexSpan = new RegExp(
      `#my-svg \\.${cls} span\\{fill:${colors.oldFill}!important;stroke:${colors.oldStroke}!important;stroke-width:2px!important;color:${colors.oldText}!important;\\}`,
      "gi"
    );
    ...
    const regexTspan = new RegExp(
      `#my-svg \\.${cls} tspan\\{fill:${colors.oldText}!important;\\}`,
      "gi"
    );
