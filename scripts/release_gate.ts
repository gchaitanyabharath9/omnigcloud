...
function printResults(results: PaperAnalysis[]) {
  ...
  results.forEach((r) => {
    ...
    console.log(
      `  Keywords: ${r.keywordCount} (${MIN_KEYWORDS}-${MAX_KEYWORDS}) ${r.checks.keywords ? "✓" : "✗"}`
    );
    console.log(
      `  Abstract: ${r.abstractWordCount} words (${MIN_ABSTRACT_WORDS}-${MAX_ABSTRACT_WORDS}) ${r.checks.abstract ? "✓" : "✗"}`
    );
  ...
  console.log("╔══════════════════════════════════════════════════════════════════════╗");
  console.log(
    `║  OVERALL STATUS: ${allPass ? "✓ READY FOR SUBMISSION" : "✗ NOT READY"}                         ║`
  );
  console.log("╚══════════════════════════════════════════════════════════════════════╝\n");
  ...
  if (!allPass) {
    ...
    results
      ...
      .forEach((r) => {
        ...
        if (!r.checks.diagrams)
          console.log(`    • Needs ${MIN_DIAGRAMS - r.diagramCount} more diagrams`);
        ...
        if (!r.checks.abstract)
          console.log(`    • Fix abstract length (current: ${r.abstractWordCount} words)`);
        if (!r.checks.noForbiddenTerms)
          console.log(`    • Remove forbidden terms (USCIS/EB-1A/exhibit/petition)`);
      });
    console.log("");
  }
}
