...
test.describe("i18n & Release Gate", () => {
  test.afterAll(() => {
    ...
    const report = [
      ...
      "|-----|------|-----------|-----------------|-------|",
      ...seoResults.map(
        (r) => `| ${r.url} | ${r.lang} | ${r.canonical} | ${r.hreflangs.length} | ${r.title} |`
      ),
    ].join("\n");
      ...
      test(`Audit CRITICAL URL: ${url}`, async ({ page }) => {
        ...
        if (isError) {
          await page.screenshot({
            path: `qa-i18n/failures/error_${parsedUrl.pathname.replace(/\//g, "_")}.png`,
          });
        ...
        if (consoleErrors.length > 0) {
          await page.screenshot({
            path: `qa-i18n/failures/console_${parsedUrl.pathname.replace(/\//g, "_")}.png`,
          });
        ...
        const hreflangs = await page
          ...
          .evaluateAll((links: Array<Element>) =>
            links.map((l) => l.getAttribute("hreflang") || "")
          );
        const description =
          (await page.getAttribute('meta[name="description"]', "content")) || "MISSING";

        seoResults.push({
          url,
          lang: htmlLang || "missing",
          canonical,
          hreflangs,
          title: pageTitle,
          description,
        ...
        const internalLinks = await page
          .locator('a[href^="/"]')
          .evaluateAll((elements: Array<Element>) =>
        ...
        if (!process.env.CI || isHome) {
          ...
          await page.screenshot({
            path: `qa-i18n/smoke/desktop_${locale}_${pathSuffix.replace(/\//g, "_")}.png`,
          });

          if (viewport && viewport.width < 768) {
            const menuBtn = page
              .locator('button[aria-label*="menu"], button[class*="menu-toggle"]')
              .first();
            if (await menuBtn.isVisible()) {
              await menuBtn.click();
              await page.waitForTimeout(300);
              await page.screenshot({
                path: `qa-i18n/smoke/mobile_menu_${locale}_${pathSuffix.replace(/\//g, "_")}.png`,
              });
            }
          }
        }
