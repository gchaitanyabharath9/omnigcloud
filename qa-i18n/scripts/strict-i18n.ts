...
function validate() {
  ...
  LOCALES.forEach((locale) => {
    ...
    if (!fs.existsSync(filePath)) {
      allErrors.push({
        ...
        key: "FILE",
        value: "MISSING",
        reason: "Translation file is missing",
      });
      return;
    }
    ...
    tier1Keys.forEach((key) => {
      ...
      const hasPlaceholder =
        BLOCKLIST_PLACEHOLDERS.some((p) => trimmed.toUpperCase().includes(p)) ||
        BLOCKLIST_REGEX.test(trimmed);
      if (hasPlaceholder) {
        allErrors.push({
          locale,
          key,
          value: val,
          reason: "Value contains placeholder/TODO string",
        });
        return;
      }
      ...
      if (enVal && trimmed === enVal.trim() && !BRAND_KEYS.includes(key)) {
        allErrors.push({
          ...
          key,
          value: val,
          reason: "Value is identical to English (no fallback allowed)",
        });
        return;
      }
    });
  ...
  const report: string[] = [
    ...
    allErrors.length === 0
      ...
      : `❌ **STATUS: FAIL** - Found ${allErrors.length} validation errors.`,
    "",
  ];

  if (allErrors.length > 0) {
    const grouped = allErrors.reduce(
      (acc, err) => {
    ...
    report.push(
      "2. Provide a real, unique translation for the failing keys (no TODOs, no English copy)."
    );
